      ...
      <Card className={cn("dark:text-white text-black", color)}>
        <CardContent className="p-6 space-y-6">
          ...
          <div className="space-y-4">
            ...
            <a href={formatWhatsAppLink(server.telefonePrincipal)} target="_blank" rel="noopener noreferrer" className="flex items-center gap-4 text-base dark:text-white text-black hover:text-white/80">
          ...
          <div className="grid grid-cols-2 gap-4">
            <Button variant="outline" asChild className="dark:bg-white/10 bg-black/10 hover:dark:bg-white/20 hover:bg-black/20 dark:border-white/20 border-black/20 dark:text-white text-black">
              <Link href={`/servidores/novo?id=${id}`}>
        ...
        <Tabs defaultValue="ficha" className="w-full flex-1 flex flex-col">
            ...
            <Card className={cn(color, "dark:text-white text-black")}>
              <CardHeader className="flex flex-row items-center justify-between">
                <div className="flex items-center gap-2">
                  <CalendarX className="h-6 w-6 dark:text-white text-black" />
                  <CardTitle className="text-lg">Faltas</CardTitle>
                </div>
                 <Dialog open={isFaltaDialogOpen} onOpenChange={setIsFaltaDialogOpen}>
                  <DialogTrigger asChild>
                    <Button size="sm" onClick={() => setEditingFalta(null)} className={"dark:bg-white/10 bg-black/10 hover:dark:bg-white/20 hover:bg-black/20 dark:border-white/20 border-black/20 dark:text-white text-black"}>
                  ...
                  <DialogContent className={cn("sm:max-w-md", color, "dark:text-white text-black")}>
                       ...
                       <div className="space-y-2">
                          ...
                          <div className="grid grid-cols-3 gap-2">
                            <Input
                              ...
                              maxLength={2}
                              className="dark:bg-black/40 bg-white/40 dark:border-white/20 border-black/20 dark:placeholder:text-white/60 placeholder:text-black/60"
                            />
                             <Input
                              type="number"
                              placeholder="Mês"
                    ...
                    <DialogFooter>
                      <Button variant="ghost" onClick={() => setIsFaltaDialogOpen(false)} className="dark:text-white text-black hover:dark:bg-white/10 hover:bg-black/10">Cancelar</Button>
                      <Button onClick={handleSaveFalta} className="dark:bg-white/90 bg-black/90 hover:dark:bg-white hover:bg-black dark:text-black text-white">Salvar</Button>
                    </DialogFooter>
                  </DialogContent>
                </Dialog>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="grid grid-cols-2 gap-4 mb-4">
                    <Select value={selectedFaltaYear} onValueChange={setSelectedFaltaYear}>
                        <SelectTrigger className="dark:bg-black/20 bg-white/20 dark:border-white/20 border-black/20">
                ...
                {isLoadingFaltas ? <p>Carregando faltas...</p> : (filteredFaltas && filteredFaltas.length > 0) ? (
                  <Table>
                    <TableHeader className="hidden md:table-header-group">
                      <TableRow className="dark:border-white/20 border-black/20">
                        <TableHead className="dark:text-white text-black">Data</TableHead>
                        <TableHead className="dark:text-white text-black">Descrição</TableHead>
                        <TableHead className="text-right dark:text-white text-black">Ações</TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      {filteredFaltas.map((falta) => (
                        <TableRow key={falta.id} className="flex flex-col md:table-row p-4 md:p-0 dark:border-white/20 border-black/20 last:border-b-0 md:border-b">
                          <TableCell className="font-medium p-0 md:p-4">
                            <span className="md:hidden font-semibold">Data: </span>{falta.date}
                          </TableCell>
                          <TableCell className="dark:text-white/80 text-black/80 p-0 md:p-4">
                             <span className="md:hidden font-semibold dark:text-white text-black">Descrição: </span>{falta.reason || 'Sem justificativa'}
                          </TableCell>
                          <TableCell className="p-0 md:p-4 mt-2 md:mt-0 text-right">
                             <div className="flex items-center justify-end gap-2">
                                <Button variant="ghost" size="icon" onClick={() => { setEditingFalta(falta); setIsFaltaDialogOpen(true); }} className="dark:text-white text-black hover:dark:bg-white/10 hover:bg-black/10">
                  ...
                  <p className="text-center dark:text-white/80 text-black/80 py-4">Nenhuma falta registrada para este período.</p>
                )}
              </CardContent>
            </Card>
          </TabsContent>
              ...
              <CardHeader className="flex flex-row items-center justify-between">
                <div className="flex items-center gap-2">
                 ...
                 <Dialog open={isLicencaDialogOpen} onOpenChange={setIsLicencaDialogOpen}>
                  <DialogTrigger asChild>
                    <Button size="sm" onClick={() => setEditingLicenca(null)} className="dark:bg-white/10 bg-black/10 hover:dark:bg-white/20 hover:bg-black/20 dark:border-white/20 border-black/20 dark:text-white text-black">
                      <PlusCircle className="h-4 w-4 mr-2" />
                      Adicionar Licença
                    </Button>
                  </DialogTrigger>
                  <DialogContent className={cn("sm:max-w-md", color, "dark:text-white text-black")}>
                    ...
                    <div className="space-y-4 py-4">
                       <div className="space-y-2">
                            ...
                            <Select value={licencaType} onValueChange={setLicencaType}>
                                <SelectTrigger id="licenca-type" className="dark:bg-black/40 bg-white/40 dark:border-white/20 border-black/20">
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="grid grid-cols-3 gap-2">
                                <Input type="number" placeholder="Dia" value={licencaStartDia} onChange={(e) => setLicencaStartDia(e.target.value)} maxLength={2} className="dark:bg-black/40 bg-white/40 dark:border-white/20 border-black/20 dark:placeholder:text-white/60 placeholder:text-black/60" />
                                <Input type="number" placeholder="Mês" value={licencaStartMes} onChange={(e) => setLicencaStartMes(e.target.value)} maxLength={2} className="dark:bg-black/40 bg-white/40 dark:border-white/20 border-black/20 dark:placeholder:text-white/60 placeholder:text-black/60" />
                                <Input type="number" placeholder="Ano" value={licencaStartAno} onChange={(e) => setLicencaStartAno(e.target.value)} maxLength={4} className="dark:bg-black/40 bg-white/40 dark:border-white/20 border-black/20 dark:placeholder:text-white/60 placeholder:text-black/60" />
                            </div>
                        </div>
                        <div className="space-y-2">
                            <Label>Data de Fim</Label>
                            <div className="grid grid-cols-3 gap-2">
                                <Input type="number" placeholder="Dia" value={licencaEndDia} onChange={(e) => setLicencaEndDia(e.target.value)} maxLength={2} className="dark:bg-black/40 bg-white/40 dark:border-white/20 border-black/20 dark:placeholder:text-white/60 placeholder:text-black/60" />
                                <Input type="number" placeholder="Mês" value={licencaEndMes} onChange={(e) => setLicencaEndMes(e.target.value)} maxLength={2} className="dark:bg-black/40 bg-white/40 dark:border-white/20 border-black/20 dark:placeholder:text-white/60 placeholder:text-black/60" />
                                <Input type="number" placeholder="Ano" value={licencaEndAno} onChange={(e) => setLicencaEndAno(e.target.value)} maxLength={4} className="dark:bg-black/40 bg-white/40 dark:border-white/20 border-black/20 dark:placeholder:text-white/60 placeholder:text-black/60" />
                            </div>
                        </div>
                       {licencaType !== 'outro' && (
                        <div className="space-y-2">
                    ...
                    <DialogFooter>
                      <Button variant="ghost" onClick={() => setIsLicencaDialogOpen(false)} className="dark:text-white text-black hover:dark:bg-white/10 hover:bg-black/10">Cancelar</Button>
                      <Button onClick={handleSaveLicenca} className="dark:bg-white/90 bg-black/90 hover:dark:bg-white hover:bg-black dark:text-black text-white">Salvar</Button>
                    </DialogFooter>
                  </DialogContent>
                </Dialog>
              </CardHeader>
              <CardContent className="space-y-4">
                ...
                {isLoadingLicencas ? <p>Carregando licenças...</p> : (filteredLicencas && filteredLicencas.length > 0) ? (
                  <Table>
                    <TableHeader className="hidden md:table-header-group">
                      <TableRow className="dark:border-white/20 border-black/20">
                        <TableHead className="dark:text-white text-black">Tipo</TableHead>
                        <TableHead className="dark:text-white text-black">Período</TableHead>
                        <TableHead className="dark:text-white text-black">Descrição</TableHead>
                        <TableHead className="text-right dark:text-white text-black">Ações</TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      {filteredLicencas.map((licenca) => (
                        <TableRow key={licenca.id} className="flex flex-col md:table-row p-4 md:p-0 dark:border-white/20 border-black/20 last:border-b-0 md:border-b">
                          ...
                          <TableCell className="dark:text-white/80 text-black/80 p-0 md:p-4">
                            <span className="md:hidden font-semibold dark:text-white text-black">Descrição: </span>{licenca.reason || '-'}
                          </TableCell>
                          <TableCell className="p-0 md:p-4 mt-2 md:mt-0 text-right">
                             <div className="flex items-center justify-end gap-2">
                                <Button variant="ghost" size="icon" onClick={() => { setEditingLicenca(licenca); setIsLicencaDialogOpen(true); }} className="dark:text-white text-black hover:dark:bg-white/10 hover:bg-black/10">
                                  <Edit className="h-5 w-5" />
                                </Button>
                                <AlertDialog>
                                  <AlertDialogTrigger asChild>
                  ...
                  <p className="text-center dark:text-white/80 text-black/80 py-4">Nenhuma licença registrada para este período.</p>
                )}
              </CardContent>
            </Card>
          </TabsContent>
          <TabsContent value="ferias" className="mt-8 md:mt-10">
            <Card className={cn(color, "dark:text-white text-black")}>
                <CardHeader className="flex flex-row items-center justify-between">
                    <div className="flex items-center gap-2">
                        <CalendarDays className="h-6 w-6 dark:text-white text-black" />
                        <CardTitle className="text-lg">Férias</CardTitle>
                    </div>
                    <Dialog open={isFeriaDialogOpen} onOpenChange={setIsFeriaDialogOpen}>
                        <DialogTrigger asChild>
                            <Button size="sm" onClick={() => setEditingFeria(null)} className="dark:bg-white/10 bg-black/10 hover:dark:bg-white/20 hover:bg-black/20 dark:border-white/20 border-black/20 dark:text-white text-black">
                                <PlusCircle className="h-4 w-4 mr-2" />
                                Adicionar Férias
                            </Button>
                        </DialogTrigger>
                        <DialogContent className={cn("sm:max-w-lg", color, "dark:text-white text-black")}>
                            ...
                            <div className="space-y-4 py-4 max-h-[60vh] overflow-y-auto pr-2">
                                ...
                                { !editingFeria &&
                                  <div className="space-y-2">
                                      ...
                                      <Select value={feriasParcelamento} onValueChange={setFeriasParcelamento}>
                                          <SelectTrigger id="ferias-parcelamento" className="dark:bg-black/40 bg-white/40 dark:border-white/20 border-black/20">
                                ...
                                {feriasPeriodos.map((periodo, index) => (
                                    <div key={index} className="space-y-3 p-3 border dark:border-white/20 border-black/20 rounded-md relative">
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <div className="grid grid-cols-3 gap-2">
                                                <Input type="number" placeholder="Dia" value={periodo.startDia} onChange={(e) => handleFeriasPeriodoChange(index, 'startDia', e.target.value)} maxLength={2} className="dark:bg-black/40 bg-white/40 dark:border-white/20 border-black/20 dark:placeholder:text-white/60 placeholder:text-black/60"/>
                                                <Input type="number" placeholder="Mês" value={periodo.startMes} onChange={(e) => handleFeriasPeriodoChange(index, 'startMes', e.target.value)} maxLength={2} className="dark:bg-black/40 bg-white/40 dark:border-white/20 border-black/20 dark:placeholder:text-white/60 placeholder:text-black/60"/>
                                                <Input type="number" placeholder="Ano" value={periodo.startAno} onChange={(e) => handleFeriasPeriodoChange(index, 'startAno', e.target.value)} maxLength={4} className="dark:bg-black/40 bg-white/40 dark:border-white/20 border-black/20 dark:placeholder:text-white/60 placeholder:text-black/60"/>
                                            </div>
                                        </div>
                                        <div className="space-y-2">
                                            <Label className="text-xs">Data de Fim</Label>
                                            <div className="grid grid-cols-3 gap-2">
                                                <Input type="number" placeholder="Dia" value={periodo.endDia} onChange={(e) => handleFeriasPeriodoChange(index, 'endDia', e.target.value)} maxLength={2} className="dark:bg-black/40 bg-white/40 dark:border-white/20 border-black/20 dark:placeholder:text-white/60 placeholder:text-black/60"/>
                                                <Input type="number" placeholder="Mês" value={periodo.endMes} onChange={(e) => handleFeriasPeriodoChange(index, 'endMes', e.target.value)} maxLength={2} className="dark:bg-black/40 bg-white/40 dark:border-white/20 border-black/20 dark:placeholder:text-white/60 placeholder:text-black/60"/>
                                                <Input type="number" placeholder="Ano" value={periodo.endAno} onChange={(e) => handleFeriasPeriodoChange(index, 'endAno', e.target.value)} maxLength={4} className="dark:bg-black/40 bg-white/40 dark:border-white/20 border-black/20 dark:placeholder:text-white/60 placeholder:text-black/60"/>
                                            </div>
                                        </div>
                                    </div>
                                ))}
                                ...
                                {feriasParcelamento === 'custom' && !editingFeria && (
                                    <Button type="button" variant="outline" size="sm" onClick={addFeriasPeriodo} className="w-full dark:bg-white/10 bg-black/10 hover:dark:bg-white/20 hover:bg-black/20 dark:border-white/20 border-black/20 dark:text-white text-black">
                                        <PlusCircle className="h-4 w-4 mr-2" />
                                        Adicionar Período
                                    </Button>
                                )}
                            ...
                            <DialogFooter>
                                <Button variant="ghost" onClick={() => setIsFeriaDialogOpen(false)} className="dark:text-white text-black hover:dark:bg-white/10 hover:bg-black/10">Cancelar</Button>
                                <Button onClick={handleSaveFeria} className="dark:bg-white/90 bg-black/90 hover:dark:bg-white hover:bg-black dark:text-black text-white">Salvar</Button>
                            </DialogFooter>
                        </DialogContent>
                    </Dialog>
                </CardHeader>
                <CardContent className="space-y-4">
                    ...
                    {isLoadingFerias ? <p>Carregando férias...</p> : (filteredFerias && filteredFerias.length > 0) ? (
                        <Table>
                            <TableHeader className="hidden md:table-header-group">
                                <TableRow className="dark:border-white/20 border-black/20">
                                    <TableHead className="dark:text-white text-black">Período Aquisitivo</TableHead>
                                    <TableHead className="dark:text-white text-black">Período de Gozo</TableHead>
                                    <TableHead className="text-right dark:text-white text-black">Ações</TableHead>
                                </TableRow>
                            </TableHeader>
                            <TableBody>
                                {filteredFerias.map((feria) => (
                                    <TableRow key={feria.id} className="flex flex-col md:table-row p-4 md:p-0 dark:border-white/20 border-black/20 last:border-b-0 md:border-b">
                                        <TableCell className="p-0 md:p-4 font-medium">
                                        ...
                                        <TableCell className="p-0 md:p-4 mt-2 md:mt-0 text-right">
                                            <div className="flex items-center justify-end gap-2">
                                                <Button variant="ghost" size="icon" onClick={() => { setEditingFeria(feria); setIsFeriaDialogOpen(true); }} className="dark:text-white text-black hover:dark:bg-white/10 hover:bg-black/10">
                                                    <Edit className="h-5 w-5" />
                                                </Button>
                                                <AlertDialog>
                                                    <AlertDialogTrigger asChild>
                        ...
                        <p className="text-center dark:text-white/80 text-black/80 py-4">Nenhum registro de férias para este período.</p>
                    )}
                </CardContent>
            </Card>
          </TabsContent>
